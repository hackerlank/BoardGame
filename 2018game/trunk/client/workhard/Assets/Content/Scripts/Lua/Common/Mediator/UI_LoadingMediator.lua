--[[
  * ui mediator class:: UI_LoadingMediator
...
local mediator = class('UI_LoadingMediator',pm.Mediator)
...
function mediator:listNotificationInterests()
    local notification = {}
    table.insert(notification, Common.ON_LOADING_LEVEL)
	return notification
end

--handle notification events
function mediator:handleNotification(notification)
    if self.bOpened == false then 
        return
    end 
    ...
    if Common.ON_LOADING_LEVEL == name then 
        self.viewComponent:FreshWindow(body)
    end 
end
...
function mediator:Opened(param)
    self.bOpened = true
    self.viewComponent:FreshWindow(0)
