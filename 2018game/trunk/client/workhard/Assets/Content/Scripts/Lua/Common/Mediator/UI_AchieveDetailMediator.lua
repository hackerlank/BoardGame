--[[
  * ui mediator class:: UI_AchieveDetailMediator
...
local mediator = class('UI_AchieveDetailMediator',pm.Mediator)
...
function mediator:Opened(param)
    ...
    local roundRecords = ListObject()
    local room_id = nil 
    local game_type = nil 
    local record_id = nil 
    local hall_service_id = nil 
    local rules = nil 
    if param and param:GetMenuParam() ~= nil then 
        local menu_param = param:GetMenuParam()
        room_id = menu_param.roomId
        game_type = menu_param.game_type 
        record_id = menu_param.record_id
        hall_service_id = menu_param.service_id
        rules = menu_param.rules
        if menu_param.round_score_infos then 
            for k,v in ipairs(menu_param.round_score_infos) do 
                roundRecords:Add(v)
            end 
        end 
    end 

    self.viewComponent:FreshWindow(roundRecords, game_type, room_id, record_id, hall_service_id, rules)
end 
