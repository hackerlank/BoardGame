--[[
  * ui mediator class:: UI_NotificationMediator
...
local mediator = class('UI_NotificationMediator',pm.Mediator)
...
function mediator:listNotificationInterests()
    local notification = {}
    table.insert(notification, Common.RENDER_NOTIFICAITON)
	return notification
end
...
function mediator:handleNotification(notification)
    ...
    if name == Common.RENDER_NOTIFICAITON then 
        self.viewComponent:RenderNotification(body.msg, body.weight)
    end 
end
