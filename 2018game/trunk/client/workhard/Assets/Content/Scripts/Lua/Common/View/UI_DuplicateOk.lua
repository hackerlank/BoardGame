...
--close self param
local CLOSE_SELF_PARAM = ci.GetUICloseParam().new(false, 'UI_DuplicateOk') 
...
local function onClickCloseBtn()
    if m_OpenAnimTween ~= nil then 
        ...
        m_OpenAnimTween = DoTweenPathLuaUtil.DoScale(m_RootPanel, 0.1, 0.1, 0.1, MENU_OPEN_ANIM_TIME)
...
local function onClickWeChatBtn()
    facade:sendNotification(Common.LAUNCH_THIRD_APP, ELaunchApp.ELA_WeChat)
end 

local function onClickQQBtn()
    facade:sendNotification(Common.LAUNCH_THIRD_APP, ELaunchApp.ELA_QQ)
end 

local function onClickAlipayBtn()
    facade:sendNotification(Common.LAUNCH_THIRD_APP, ELaunchApp.ELA_Alipay)
end 
...
local function BindCallbacks()
    ...
    btn = m_RootPanel:Find("btns/btn_wechat"):GetComponent('Button')
    if btn ~= nil then 
        btn.onClick:AddListener(onClickWeChatBtn)
        table.insert(tb_btns, btn)
    end 

    btn = m_RootPanel:Find("btns/btn_qq"):GetComponent('Button')
    if btn ~= nil then 
        btn.onClick:AddListener(onClickQQBtn)
        table.insert(tb_btns, btn)
    end 

    btn = m_RootPanel:Find("btns/btn_alipay"):GetComponent('Button')
    if btn ~= nil then 
        btn.onClick:AddListener(onClickAlipayBtn)
        table.insert(tb_btns, btn)
    end 
...
function tbclass:Init()
    if transform == nil then 
        LogError('[UI_DuplicateOk.Init]:: missing transform')
        return
    end 
...
function tbclass:NtfLaunchAppOk()
    facade:sendNotification(Common.CLOSE_UI_COMMAND, CLOSE_SELF_PARAM)
end 
