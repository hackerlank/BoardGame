...
local m_LoadingTrans = nil 
local m_LoadingAnim = nil 

--close self param
local CLOSE_SELF_PARAM = ci.GetUICloseParam().new(false, 'UI_Connect') 

--close self can restore param
local CLOSE_CAN_RESTORE_PARAM = ci.GetUICloseParam().new(true, 'UI_Connect') 
...
function tbclass:Init()
    if transform == nil then 
        LogError('[UI_Connect.Init]:: missing transform')
        return
    end 
    ...
    txt_ps = m_RootPanel:Find("txt_tip"):GetComponent("Text")
    txt_ps.text = ""

    m_LoadingTrans = m_RootPanel:Find("static_anim")

    if m_LoadingAnim == nil then 
        m_LoadingAnim = DoTweenPathLuaUtil.DoLocalRotation(m_RootPanel, 0, 0, -360, 2)
        DoTweenPathLuaUtil.SetEaseTweener(m_OpenAnimTween, DG.Tweening.Ease.Linear)
        DoTweenPathLuaUtil.SetAutoKill(m_LoadingAnim, false)
        DoTweenPathLuaUtil.DOPause(m_LoadingTrans)
    end 
...
function tbclass:SafeRelease()
    ...
    if m_LoadingAnim then 
        DoTweenPathLuaUtil.Kill(m_LoadingAnim, true)
        m_LoadingAnim = nil 
    end 
    m_RootPanel = nil 
...
function  tbclass:UpdateLoginMenu(tip)
    -- body
    if tip then 
        m_RootPanel.gameObject:SetActive(true)
        txt_ps.text = tostring(tip)
        if m_LoadingAnim then 
            DoTweenPathLuaUtil.DORestart(m_LoadingTrans)
        end 
    else 
