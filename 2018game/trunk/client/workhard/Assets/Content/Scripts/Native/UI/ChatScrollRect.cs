...
public class ChatScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ISelectHandler
{
    ...
    private bool IsDealHiddenItem()
    {
        ...
        if (bMoveToEnd)
        {
            ...
            {
                float diff = cacheDiff + cachedDefaultLocation - m_Content.anchoredPosition.y;
                diff = (int)(diff * 100) / 100;
                bHidden = diff <= Mathf.Epsilon;
            }
        }
        else
        {
    ...
    [SLua.DoNotToLua]
    protected Tweener m_RegisterAnim = null;
    ...
    [SLua.DoNotToLua]
    protected void PlayRegisterItemAnim()
    {
        float start_y = m_Content.anchoredPosition.y;
        float end_y = start_y + cellInternal;

        if(m_RegisterAnim == null)
        {
            m_RegisterAnim = m_Content.gameObject.transform.DOMoveY(end_y, 0.1f);
            m_RegisterAnim.OnComplete(OnCompleted);
        }
    }

    [SLua.DoNotToLua]
   
    private IEnumerator DoMoveY(float dis)
    {
        float total = dis;
        float speed = (float)(dis / 0.08);
        Vector2 startPos = m_Content.anchoredPosition;
        while(total > 0)
        {
            float tmp = (float)(speed * 0.03);
                if(total - tmp <= 0)
            {
                tmp = total;
                total = 0;
            ...
            startPos.y += tmp;
            m_Content.anchoredPosition = startPos;
    ...
    public void RegisterItem(object item)
    {
        ...
        if(m_pages.Count >= minColumnOrRaw && currentEndLine != m_pages.Count -1)
        {
            m_bNeedUpdate = false;
        }
        ...
        {
            currentEndLine++;
            if (m_pages.Count > minColumnOrRaw)
            {
                currentStartLine++;

            }

            int start_idx = m_rendercount;
            int endidx = start_idx + layout.constraintCount;
            for (int idx = start_idx; idx < endidx; ++idx)
            {
                SlotItemInfo trans = m_Content.GetChild(idx).GetComponent<SlotItemInfo>();
                int real_idx = idx - start_idx;
                if (real_idx < m_pages[currentEndLine].Count)
                {
                    trans.gameObject.SetActive(true);
                    trans.info = m_pages[currentEndLine][real_idx];
                }
                else
                {
                    if (bHideBlankItem == true)
        ...
        {
            bRunLateUpdated = false;
            StartCoroutine(DoMoveY(cellInternal));
        }
        else
        {
            m_bIsRegister = false;
