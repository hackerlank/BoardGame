...
namespace UnityEditor.XCodeEditor
{
	...
	{
        private static string sdk_path = "/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneOS.platform/Developer/SDKs/iPhoneOS11.2.sdk";
        ...
        public bool Add_Other_Building_Flags(string key , string value)
        {
           // string baseKey = ""
            foreach (KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations)
            {
                buildConfig.Value.AddOtherBuildingFalg(key, value);
        ...
        public bool Remove_Building_Flags(string key)
        {
            foreach (KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations)
            {
                buildConfig.Value.RemoveBuildingFlag(key);
        ...
        public void RegisterNewLib(string fileName)
        {
            ...
            string completeLibPath =  string.Format("{0}/usr/lib/{1}",sdk_path, fileName);
            ...
            this.AddFile(completeLibPath, group, "SDKROOT", true, true);
            
           // string completePath = System.IO.Patfh.Combine("System/Library/Frameworks", fileName);
          //  this.AddFile(completePath, frameworkGroup, "SDKROOT", true, isWeak);
        }

        public void RegisterNewFramework(string framework)
        {
            ...
            string completeLibPath = string.Format("{0}/System/Library/Frameworks/{1}", sdk_path, framework);
            Debug.Log("Adding library " + completeLibPath);
            PBXGroup group = null;
            if (groups != null)
            {
		...
		{	
			...
			//plist.ReplaceKey ("<string>com.Company.${PROJECT_NAME}</string>", "<string>" + bundleIdentifier + "</string>");
			//plist.Save ();
